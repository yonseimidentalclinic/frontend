...
const AdminFaqListPage = () => {
  ...
  const fetchFaqs = useCallback(async () => {
    setLoading(true);
    try {
      const response = await api.get('/faqs'); // publicRoutes에 있는 GET /faqs 사용
  ...
  const handleUpdate = async (id) => {
    try {
      await api.put(`/admin/faqs/${id}`, editingFaq);
      setEditingFaq(null);
      fetchFaqs(); // 목록 새로고침
      ...
      {isAdding && (
        <form onSubmit={handleAddNew} className="bg-white p-6 rounded-lg shadow-md mb-8">
          ...
          <div className="space-y-4">
            <input type="text" name="category" value={newFaq.category} onChange={handleInputChange} placeholder="카테고리 (예: 임플란트)" className="w-full p-2 border rounded" />
            <input type="text" name="question" value={newFaq.question} onChange={handleInputChange} placeholder="질문" className="w-full p-2 border rounded" />
            <textarea name="answer" value={newFaq.answer} onChange={handleInputChange} placeholder="답변" rows="4" className="w-full p-2 border rounded"></textarea>
          </div>
            ...
            <li key={faq.id} className="border p-4 rounded-lg">
              {editingFaq && editingFaq.id === faq.id ? (
                ...
                <div className="space-y-2">
                  <input type="text" name="category" value={editingFaq.category} onChange={handleInputChange} className="w-full p-2 border rounded" />
                  <input type="text" name="question" value={editingFaq.question} onChange={handleInputChange} className="w-full p-2 border rounded" />
                  <textarea name="answer" value={editingFaq.answer} onChange={handleInputChange} rows="3" className="w-full p-2 border rounded"></textarea>
