...
const AdminPostListPage = () => {
  ...
  const fetchPosts = useCallback(async () => {
    setLoading(true);
    try {
      // 관리자 페이지에서는 페이지네이션 없이 모든 글을 가져옵니다.
  ...
  const handleDelete = async (id) => {
    if (window.confirm(`[ID: ${id}] 게시글을 정말로 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`)) {
      try {
              ...
              <tr key={post.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-center">
                  <div className="flex justify-center gap-4">
                    <button onClick={() => navigate(`/admin/posts/edit/${post.id}`)} title="수정 및 댓글 관리" className="text-indigo-600 hover:text-indigo-900"><Edit size={20} /></button>
                    <button onClick={() => handleDelete(post.id)} title="삭제" className="text-red-600 hover:text-red-900"><Trash2 size={20} /></button>
                  </div>
                </td>
              </tr>
            ))}
